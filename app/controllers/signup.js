...
import User from 'gooru-web/models/tenant-login';
...
export default Ember.Controller.extend(ConfigurationMixin, {
      ...
      user.validate().then(function({ validations }) {
        ...
        if (validations.get('isValid')) {
          controller
            ...
            .then(
              function(tenantList) {
                controller.set('isViewSingUp', true);
  ...
  hasGooruAccount: Ember.computed('tenantList', function() {
    let tenantLists = this.get('tenantLists');
    return !!tenantLists.findBy('tenantShortName', 'gooru');
  }),
