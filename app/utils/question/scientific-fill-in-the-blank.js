 ...
 *  ['black', '', 'blue']
 *
 * @typedef {Object} ScientificFillInTheBlankUtil
 */
const ScientificFillInTheBlankUtil = QuestionUtil.extend({
  // -------------------------------------------------------------------------
  // Observers
  ...
  getQuestionAnswers: function() {
    const answersFromText = ScientificFillInTheBlankUtil.getQuestionAnswers(
      this.get('question')
    );
...
ScientificFillInTheBlankUtil.reopenClass({
  /**
  ...
  getCorrectAnswers: function(text) {
    const regExp = ScientificFillInTheBlankUtil.FIB_REGEX.global;
    ...
    while (matches) {
      ...
      if (include) {
        let textValue = matches[2].substring(1, matches[2].length - 1);
        answers.push(textValue); // return second group
      }
  ...
  toFibText: function(text) {
    if (text) {
      const inputText = ScientificFillInTheBlankUtil.LEGACY_REGEX.text;
      const regExp = ScientificFillInTheBlankUtil.FIB_REGEX.global;
   ...
   * @return {Answer[]}
   */
  getQuestionAnswers: function(answerDetails) {
    const owner = Ember.getOwner(answerDetails);
    return answerDetails.map(function(answer) {
      return Answer.create(owner ? owner.ownerInjection() : undefined, {
        answer_text: answer.answer_text.replace(/\[[^\]]+\]/g, '[]'),
        correct_answer: ScientificFillInTheBlankUtil.getCorrectAnswers(
          answer.answer_text
        ),
        answer_type: 'text',
...
export default ScientificFillInTheBlankUtil;
