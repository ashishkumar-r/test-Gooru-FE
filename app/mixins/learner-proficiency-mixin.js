...
export default Ember.Mixin.create(visibilitySettings, TenantSettingsMixin, {
  ...
  actions: {
    ...
    onSelectCompetency(competency, domainCompetencyList) {
      ...
      competency.set('domainName', selectedDomain.domainName);
   ...
   */
  loadDataBySubject(isSubjectChanged) {
    let component = this;
    ...
    if (isSubjectChanged) {
      component.set('framework', isSubjectChanged.code);
    }
  ...
  initialSkylineMasteries(proficiencyChartData) {
    ...
    if (!masteredCompetencies && isPublic) {
      ...
      if (this.get('isNextEnable')) {
        let gradeCurrent = this.get('studentMathGrade.id');
      ...
      if (gradeLowerBoundSeq >= 1) {
        let activeGrade = taxonomyGrades.findBy('sequence', gradeLowerBoundSeq);
