/* eslint-disable */
/*eslint eqeqeq: "off"*/
/*! simplify-scorm v2.0.0 - 2021-06-03 */
(window.simplifyScorm = {}),
  ...
  (window.simplifyScorm.jsonFormatter = function e() {
    (this.jsonString = !0), delete this.toJSON;
    var t = JSON.stringify(this);
    delete this.jsonString, (this.toJSON = e);
    var r = JSON.parse(t);
    for (var i in (delete r.jsonString, r)) {
      r.hasOwnProperty(i) && 0 === i.indexOf('_') && delete r[i];
    }
    return r;
  }),
  (function() {
    window.simplifyScorm.BaseAPI = e;
    var n = window.simplifyScorm.constants;
    function e() {
      var e = this;
      (e.currentState = n.STATE_NOT_INITIALIZED),
        (e.lastErrorCode = '0'),
        (e.apiLog = t),
        (e.apiLogLevel = n.LOG_LEVEL_ERROR),
        (e.clearSCORMError = r),
        (e.getLmsErrorMessageDetails = i),
        (e.isInitialized = s),
        (e.isNotInitialized = a),
        (e.isTerminated = o),
        (e.listenerArray = []),
        (e.on = l),
        (e.processListeners = _),
        (e.throwSCORMError = c);
    }
    function t(e, t, r, i) {
      if (
        ((r = (function(e, t, r) {
          for (var i = '', n = 20 - (i += e).length, s = 0; s < n; s++) {
            i += ' ';
          }
          if (((i += ': '), t)) {
            n = 70 - (i += t).length;
        ...
        switch (i) {
          case n.LOG_LEVEL_ERROR:
            console.error(r);
            break;
          case n.LOG_LEVEL_WARNING:
            console.warn(r);
            break;
          case n.LOG_LEVEL_INFO:
            console.info(r);
        }
      }
    }
    function r(e) {
      e !== n.SCORM_FALSE && (this.lastErrorCode = '0');
    }
    function i(e, t) {
      return 'No error';
    }
    function s() {
      return this.currentState === n.STATE_INITIALIZED;
    }
    function a() {
      return this.currentState === n.STATE_NOT_INITIALIZED;
    }
    function o() {
      return this.currentState === n.STATE_TERMINATED;
    }
    function l(e, t) {
      if (t) {
    ...
    function _(e, t, r) {
      for (var i = 0; i < this.listenerArray.length; i++) {
        var n = this.listenerArray[i],
          ...
          a = !!n.CMIElement,
          o = n.CMIElement === t;
        !s || (a && !o) || n.callback(t, r);
      }
    }
    function c(e, t) {
      t || (t = this.getLmsErrorMessageDetails(e)),
        this.apiLog('throwSCORMError', null, `${e}: ${t}`, n.LOG_LEVEL_ERROR),
        (this.lastErrorCode = String(e));
    }
    e.reset = function() {
      (this.currentState = n.STATE_NOT_INITIALIZED),
        (this.lastErrorCode = '0'),
        (this.apiLogLevel = n.LOG_LEVEL_ERROR),
        (this.listenerArray = []);
    };
  })(),
  (function() {
    window.simplifyScorm.ScormAPI = t;
    var e = window.simplifyScorm.BaseAPI,
      c = window.simplifyScorm.constants,
      r = window.simplifyScorm.jsonFormatter;
    function t() {
      var _ = this;
      function a(e, t) {
        if (!e || '' === e) {
          return c.SCORM_FALSE;
        }
        for (
          var r = e.split('.'), i = _, n = c.SCORM_FALSE, s = 0;
          s < r.length;
          s++
        ) {
          if (s === r.length - 1) {
            i.hasOwnProperty(r[s])
              ? ((i[r[s]] = t), (n = c.SCORM_TRUE))
              : _.throwSCORMError(
                  101,
                  `setCMIValue did not find an element for: ${e}`
                );
          } else {
            if (!(i = i[r[s]])) {
              _.throwSCORMError(
            ...
            if (i.hasOwnProperty('childArray')) {
              var a = parseInt(r[s + 1], 10);
              if (!isNaN(a)) {
                var o,
                  l = i.childArray[a];
                if (l) {
                  ...
                  -1 < e.indexOf('cmi.objectives')
                    ? (o = new h(_))
                    : -1 < e.indexOf('.correct_responses')
                    ? (o = new m(_))
                    : -1 < e.indexOf('.objectives')
                    ? (o = new u(_))
                    : -1 < e.indexOf('cmi.interactions') && (o = new d(_)),
                    o
                      ? (i.childArray.push(o), (i = o))
                      : _.throwSCORMError(
                          101,
                          `Cannot create new sub entity: ${e}`
                        );
                }
                s++;
              }
        ...
        return (
          n === c.SCORM_FALSE &&
            _.apiLog(
              'LMSSetValue',
              null,
              `There was an error setting the value for: ${e}, value of: ${t}`,
              c.LOG_LEVEL_WARNING
            ),
          n
        );
      ...
      return (
        e.call(_),
        (_.LMSInitialize = function() {
          var e = c.SCORM_FALSE;
          _.isInitialized()
            ? _.throwSCORMError(101, 'LMS was already initialized!')
            : _.isTerminated()
            ? _.throwSCORMError(101, 'LMS is already finished!')
            : ((_.currentState = c.STATE_INITIALIZED),
              (e = c.SCORM_TRUE),
              _.processListeners('LMSInitialize'));
          return (
            _.apiLog('LMSInitialize', null, `returned: ${e}`, c.LOG_LEVEL_INFO),
            _.clearSCORMError(e),
            e
          );
        }),
        (_.LMSFinish = function() {
          var e = c.SCORM_FALSE;
          _.checkState() &&
            ((_.currentState = c.STATE_TERMINATED),
            (e = c.SCORM_TRUE),
            _.processListeners('LMSFinish'));
          return (
            _.apiLog('LMSFinish', null, `returned: ${e}`, c.LOG_LEVEL_INFO),
            _.clearSCORMError(e),
            e
          );
        }),
        (_.LMSGetValue = function(e) {
          var t = '';
          _.checkState() &&
            ((t = (function(e) {
              if (!e || '' === e) {
                ...
                (i = t[n]),
                  n === t.length - 1 &&
                    (r.hasOwnProperty(t[n]) ||
                      _.throwSCORMError(
                        101,
                        `getCMIValue did not find a value for: ${e}`
                      )),
                  (r = r[t[n]]);
              }
              return null == r
                ? ('_children' === i
                    ? _.throwSCORMError(202)
                    : '_count' === i && _.throwSCORMError(203),
                  '')
                : r;
            })(e)),
            _.processListeners('LMSGetValue', e));
          return (
            _.apiLog('LMSGetValue', e, `: returned: ${t}`, c.LOG_LEVEL_INFO),
            _.clearSCORMError(t),
            t
          );
        }),
        (_.LMSSetValue = function(e, t) {
          var r = '';
          _.checkState() &&
            ((r = a(e, t)), _.processListeners('LMSSetValue', e, t));
          return (
            _.apiLog(
              'LMSSetValue',
              e,
        ...
        (_.LMSCommit = function() {
          var e = c.SCORM_FALSE;
          _.checkState() &&
            ((e = c.SCORM_TRUE), _.processListeners('LMSCommit'));
          return (
            _.apiLog('LMSCommit', null, `returned: ${e}`, c.LOG_LEVEL_INFO),
            _.clearSCORMError(e),
            e
          );
        }),
        (_.LMSGetLastError = function() {
          var e = _.lastErrorCode;
          return (
            _.processListeners('LMSGetLastError'),
            _.apiLog(
              'LMSGetLastError',
              null,
              `returned: ${e}`,
        ...
        (_.LMSGetErrorString = function(e) {
          var t = '';
          null !== e &&
            '' !== e &&
            ((t = _.getLmsErrorMessageDetails(e)),
            _.processListeners('LMSGetErrorString'));
          return (
            _.apiLog(
              'LMSGetErrorString',
              null,
        ...
        (_.LMSGetDiagnostic = function(e) {
          var t = '';
          null !== e &&
            '' !== e &&
            ((t = _.getLmsErrorMessageDetails(e, !0)),
            _.processListeners('LMSGetDiagnostic'));
          return (
            _.apiLog(
              'LMSGetDiagnostic',
              null,
        ...
        (_.cmi = new i(_)),
        (_.checkState = function() {
          return !!this.isInitialized() || (this.throwSCORMError(301), !1);
        }),
        (_.getLmsErrorMessageDetails = function(e, t) {
          var r = '',
            i = '';
          switch ((e = String(e))) {
            case '101':
        ...
        (_.loadFromJSON = function(e, t) {
          if (!_.isNotInitialized()) {
            return void console.error(
              'loadFromJSON can only be called before the call to LMSInitialize.'
            );
          }
          for (var r in ((t = t || 'cmi'), e)) {
            if (e.hasOwnProperty(r) && e[r]) {
              var i = `${t}.${r}`,
                n = e[r];
              if (n.childArray) {
                for (var s = 0; s < n.childArray.length; s++) {
                  _.loadFromJSON(n.childArray[s], `${i}.${s}`);
                }
              } else {
                n.constructor === Object ? _.loadFromJSON(n, i) : a(i, n);
              }
            }
          }
        }),
        (_.reset = function() {
          e.reset.call(_), (_.cmi = new i(_));
        }),
        _
      );
    }
    ...
    function u(e) {
      return {
        _id: '',
        get id() {
          return this.jsonString ? this._id : e.throwSCORMError(404);
        },
        set id(e) {
          this._id = e;
        },
    ...
    function m(e) {
      return {
        _pattern: '',
        get pattern() {
          return this.jsonString ? this._pattern : e.throwSCORMError(404);
        },
        set pattern(e) {
          this._pattern = e;
        },
  ...
  (function() {
    window.simplifyScorm.ScormAPI2004 = t;
    var e = window.simplifyScorm.BaseAPI,
      d = window.simplifyScorm.constants,
      r = window.simplifyScorm.jsonFormatter;
    function t() {
      var c = this;
      function a(e, t) {
        if (!e || '' === e) {
          return d.SCORM_FALSE;
        }
        for (
          var r = e.split('.'), i = c, n = d.SCORM_FALSE, s = 0;
          s < r.length;
          s++
        ) {
          var a = r[s];
          if (s === r.length - 1) {
            ...
            'function' == typeof i._isTargetValid
              ? c.throwSCORMError(404)
              : i.hasOwnProperty(a)
              ? ((i[a] = t), 0 == c.lastErrorCode && (n = d.SCORM_TRUE))
              : c.throwSCORMError(
                  401,
                  `The data model element passed to SetValue (${e}) is not a valid SCORM data model element.`
                );
          } else {
            if (!(i = i[a])) {
              c.throwSCORMError(
            ...
            if (i.hasOwnProperty('childArray')) {
              ...
              if (!isNaN(o)) {
                ...
                if (_) {
                  ...
                  -1 < e.indexOf('cmi.comments_from_learner')
                    ? (l = new h(c))
                    : -1 < e.indexOf('cmi.comments_from_lms')
                    ? (l = new u(c))
                    : -1 < e.indexOf('cmi.objectives')
                    ? (l = new p(c))
                    : -1 < e.indexOf('.correct_responses')
                    ...
                    : -1 < e.indexOf('.objectives')
                    ? (l = new S(c))
                    : -1 < e.indexOf('cmi.interactions') && (l = new m(c)),
                    l
                      ? (i.childArray.push(l), (i = l))
                      : c.throwSCORMError(
                          401,
                          `The data model element passed to SetValue (${e}) is not a valid SCORM data model element.`
                        );
      ...
      return (
        ...
        (c.Initialize = function() {
          var e = d.SCORM_FALSE;
          c.isInitialized()
            ? c.throwSCORMError(103)
            : c.isTerminated()
            ? c.throwSCORMError(104)
            : ((c.currentState = d.STATE_INITIALIZED),
              (c.lastErrorCode = 0),
              (e = d.SCORM_TRUE),
              c.processListeners('Initialize'));
          return (
            c.apiLog('Initialize', null, `returned: ${e}`, d.LOG_LEVEL_INFO),
            c.clearSCORMError(e),
            e
          );
        }),
        (c.Terminate = function() {
          var e = d.SCORM_FALSE;
          c.isNotInitialized()
            ? c.throwSCORMError(112)
            : c.isTerminated()
            ? c.throwSCORMError(113)
            : ((c.currentState = d.STATE_TERMINATED),
              (c.lastErrorCode = 0),
              (e = d.SCORM_TRUE),
              c.processListeners('Terminate'));
          return (
            c.apiLog('Terminate', null, `returned: ${e}`, d.LOG_LEVEL_INFO),
            c.clearSCORMError(e),
            e
          );
        }),
        (c.GetValue = function(e) {
          var t = '';
          c.isNotInitialized()
            ? c.throwSCORMError(122)
            : c.isTerminated()
            ? c.throwSCORMError(123)
            : ((c.lastErrorCode = 0),
              (t = (function(e) {
                if (!e || '' === e) {
                  return '';
                }
                for (var t = e.split('.'), r = c, i = 0; i < t.length; i++) {
                  var n = t[i];
                  if (
                    '{target=' == n.substr(0, 8) &&
                    'function' == typeof r._isTargetValid
                  ) {
                    var s = n.substr(8, n.length - 9);
                    return r._isTargetValid(s);
                  }
                  if (!r.hasOwnProperty(n)) {
                    return (
                      c.throwSCORMError(
                        401,
                        `The data model element passed to GetValue (${e}) is not a valid SCORM data model element.`
                      ),
                      ''
                    );
                  }
              ...
              c.processListeners('GetValue', e));
          return (
            c.apiLog('GetValue', e, `: returned: ${t}`, d.LOG_LEVEL_INFO),
            c.clearSCORMError(t),
            t
          );
        }),
        (c.SetValue = function(e, t) {
          var r = '';
          c.isNotInitialized()
            ? c.throwSCORMError(132)
            : c.isTerminated()
            ? c.throwSCORMError(133)
            : ((c.lastErrorCode = 0),
              (r = a(e, t)),
              c.processListeners('SetValue', e, t));
          return (
            c.apiLog('SetValue', e, `: ${t}: result: ${r}`, d.LOG_LEVEL_INFO),
            c.clearSCORMError(r),
            r
          );
        }),
        (c.Commit = function() {
          var e = d.SCORM_FALSE;
          c.isNotInitialized()
            ? c.throwSCORMError(142)
            : c.isTerminated()
            ? c.throwSCORMError(143)
            : ((c.lastErrorCode = 0),
              (e = d.SCORM_TRUE),
              c.processListeners('Commit'));
          return (
            c.apiLog('Commit', null, `returned: ${e}`, d.LOG_LEVEL_INFO),
            c.clearSCORMError(e),
            e
          );
        }),
        (c.GetLastError = function() {
          var e = String(c.lastErrorCode);
          return (
            c.processListeners('GetLastError'),
            c.apiLog('GetLastError', null, `returned: ${e}`, d.LOG_LEVEL_INFO),
            e
          );
        }),
        (c.GetErrorString = function(e) {
          var t = '';
          null !== e &&
            '' !== e &&
            ((t = c.getLmsErrorMessageDetails(e)),
            c.processListeners('GetErrorString'));
          return (
            c.apiLog(
              'GetErrorString',
              null,
        ...
        (c.GetDiagnostic = function(e) {
          var t = '';
          null !== e &&
            '' !== e &&
            ((t = c.getLmsErrorMessageDetails(e, !0)),
            c.processListeners('GetDiagnostic'));
          return (
            c.apiLog('GetDiagnostic', null, `returned: ${t}`, d.LOG_LEVEL_INFO),
            t
          );
        }),
        (c.cmi = new i(c)),
        (c.adl = new n(c)),
        (c.getLmsErrorMessageDetails = function(e, t) {
          var r = '',
            i = '';
          switch ((e = String(e))) {
            case '0':
            ...
            case '102':
              (r = 'General Initialization Failure'),
                (i = 'Call to Initialize failed for an unknown reason.');
              break;
            case '103':
            ...
            case '111':
              (r = 'General Termination Failure'),
                (i = 'Call to Terminate failed for an unknown reason.');
              break;
            case '112':
        ...
        (c.loadFromJSON = function(e, t) {
          ...
          for (var r in ((t = t || 'cmi'), e)) {
            if (e.hasOwnProperty(r) && e[r]) {
              ...
              if (n.childArray) {
                for (var s = 0; s < n.childArray.length; s++) {
                  c.loadFromJSON(n.childArray[s], `${i}.${s}`);
                }
              } else {
                n.constructor === Object ? c.loadFromJSON(n, i) : a(i, n);
              }
            }
          }
        }),
        (c.reset = function() {
          e.reset.call(c), (c.cmi = new i(c)), (c.adl = new n(c));
        }),
        c
      );
    }
    function i(t) {
      return {
        ...
        get completion_status() {
          return (function(e) {
            var t = e._completion_status;
            e._completion_threshold && e._progress_measure
              ? (t =
                  e._progress_measure >= e._completion_threshold
                    ? 'completed'
                    : 'incomplete')
              : e._completion_threshold && (t = 'unknown');
            return t;
          })(this);
        },
        ...
        get success_status() {
          return (function(e) {
            var t = e._success_status;
            e._scaled_passing_score && e.score._scaled
              ? (t =
                  e.score._scaled >= e._scaled_passing_score
                    ? 'passed'
                    : 'failed')
              : e._scaled_passing_score && (t = 'unknown');
            return t;
          })(this);
        },
      ...
      return {
        nav: {
          ...
          request_valid: {
            ...
            choice: { _isTargetValid: s },
            jump: { _isTargetValid: a }
          }
        }
      };
    }
